...
fn main() {
    ...
    let total_health: u32 = world
        .query_iter::<Player>()
        .map(|(_, player)| player.health)
        .sum();
    let seq_time = start.elapsed();
    println!(
        "Total health (sequential): {} in {:?}",
        total_health, seq_time
    );

    // Parallel query benchmark
    println!("\n=== Parallel Query ===");
    let start = std::time::Instant::now();
    let total_health_par: u32 = world
        .par_query_iter::<Player>()
        .map(|(_, player)| player.health)
        .sum();
    let par_time = start.elapsed();
    println!(
        "Total health (parallel): {} in {:?}",
        total_health_par, par_time
    );
    ...
    println!(
        "Player stats: count={}, total_score={}, avg_score={} in {:?}",
